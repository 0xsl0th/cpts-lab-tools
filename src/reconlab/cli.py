...
app = typer.Typer(
    ...
    epilog=(
        "**Examples -- typical lab session**\n\n\n\n"
        "**1. Scaffold a workspace** -- creates the folder tree, persists target metadata, "
        "and prints a suggested /etc/hosts line so you do not have to retype the IP/host.\n\n\n\n"
        "`reconlab workspace init target --ip <ip> --host <host> --domain <domain>`\n\n\n\n"
        "**2. Run nmap into scans/** -- every workspace command reads scans/ automatically.\n\n\n\n"
        ...
        "**5. Run web content discovery into web/, then ingest** -- merges every "
        "feroxbuster/gobuster output into one deduplicated table, parallel to how suggest merges scans.\n\n\n\n"
        "`feroxbuster -u http://<ip> --json -o web/initial.json`\n\n\n\n"
        "`reconlab workspace ingest-web`\n\n\n\n"
        "**6. Record findings as you go** -- persists into report.md, replacing the placeholder finding on the first call.\n\n\n\n"
        ...
        "**Anytime: status snapshot** -- scans, methodology freshness, findings, web findings, suggested next step.\n\n\n\n"
        ...
        "`reconlab parse-web web/dirs.txt --status 200,301,403` -- filtered web table\n\n\n\n"
        "`reconlab workflow show smb` -- print one workflow as Markdown\n\n\n\n"
        "`reconlab workflow list` -- browse the workflow registry\n\n\n\n"
        "`reconlab make-hosts <ip> <host>` -- manual /etc/hosts line, no workspace\n\n\n\n"
        "Run `reconlab <command> --help` for per-command examples and full option lists."
    ),
)
